...
main()
{
  ...
  tmux set-option -ga status-left "#[bg=${green},fg=${dark_gray}]#{?client_prefix,#[bg=${yellow}],} ☺ " 

   # show battery
  tmux set-option -ga status-right "#[fg=${dark_gray},bg=${pink}] #($current_dir/battery.sh) "
 
  # show date and time
  tmux set-option -ga status-right "#[fg=${white},bg=${dark_purple}] %a %m/%d/%y %H:%M #(date +%Z) "

  # showe network and weather - didn't seem to work
